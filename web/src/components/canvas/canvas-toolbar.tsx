...
export function CanvasToolbar({
    ...
    const dockStyle = { background: theme.toolbar.panel, borderColor: theme.toolbar.border, color: theme.toolbar.item, boxShadow: colorTheme === "dark" ? "0 24px 70px rgba(0,0,0,.42), 0 0 0 1px rgba(255,255,255,.04) inset" : "0 22px 70px rgba(28,25,23,.16), 0 0 0 1px rgba(255,255,255,.65) inset" };
    const hoverStyle = { background: theme.toolbar.itemHover, color: theme.toolbar.activeText };
    const activeStyle = { background: theme.toolbar.activeBg, color: theme.toolbar.activeText };
    const tip = hovered ? toolLabel(hovered) : "";

    return (
        <div className="pointer-events-none absolute bottom-24 left-4 top-24 z-50 flex items-center">
            {tip ? <DockTip label={tip} x={tipX} theme={theme} /> : null}
            <div ref={wrapRef} className="thin-scrollbar pointer-events-auto flex max-h-full w-14 flex-col items-center gap-1 overflow-y-auto rounded-2xl border px-2 py-2 shadow-2xl backdrop-blur-2xl [&>*]:shrink-0" style={dockStyle}>
            ...
            {appearanceOpen ? (
                <div
                    className="pointer-events-auto absolute left-[72px] top-1/2 z-30 w-[264px] -translate-y-1/2 rounded-2xl border p-3 shadow-2xl backdrop-blur-2xl"
...
function Divider({ theme }: { theme: CanvasTheme }) {
    return <div className="my-1 h-px w-8" style={{ background: theme.toolbar.border }} />;
}
